...
template <typename OutType, typename TileShape, typename ClusterShape, int GroupSizeM = 1>
void launch_sm90_fp8_blockwise_scaled_mm(torch::Tensor& out, const torch::Tensor& a, const torch::Tensor& b,
  ...
  using StrideA = Stride<int64_t, cute::Int<1>, int64_t>;
  using StrideB = Stride<int64_t, cute::Int<1>, int64_t>;
  using StrideC = Stride<int64_t, cute::Int<1>, cute::Int<0>>;
  ...
  using KernelSchedule = cutlass::gemm::KernelTmaWarpSpecializedCooperativeFP8BlockScaledSubGroupMAccum<GroupSizeM>;
  ...
  StrideB stride_b{b.stride(1), cute::Int<1>{}, 0};
  StrideD stride_d{out.stride(0), cute::Int<1>{}, cute::Int<0>{}};

  typename GemmKernel::MainloopArguments mainloop_args{a_ptr, stride_a, b_ptr, stride_b, a_s_ptr, b_s_ptr};
  typename GemmKernel::EpilogueArguments epilogue_args{{}, o_ptr, stride_d, o_ptr, stride_d};
